...
Public Class ClientsPending
    ...
    Dim varVerified As Integer

    Dim editFlag As Boolean

    Private Sub ClientsPending_Load(sender As Object, e As EventArgs) Handles Me.Load
        'for datagridview
        Try
            reloaData("SELECT id, Sname, Fname, email, phonenum, bdate FROM login WHERE verified = '0'", Guna2DataGridView1)
    ...
    Private Sub Guna2Button2_Click(sender As Object, e As EventArgs) Handles Guna2Button2.Click
        Try
            strcon.Open()
            cmd.CommandText = "SELECT * FROM login WHERE id = '" & Guna2TextBox1.Text & "'"
        ...
        If Guna2ComboBox1.Text = "Verified" Then
            Try
                reloadData("Select * FROM login WHERE id = '" & varID & "'")
                If database.dt.Rows.Count > 0 Then

                    updates("UPDATE login SET verified = '1' WHERE id = '" & varID & "'")
                ...
                Else
                    MessageBox.Show("Failed to Update Account Status") 'change
                End If
            Catch ex As Exception
        ...
        ElseIf Guna2ComboBox1.Text = "Archived" Then
            Try
                reloadData("Select * FROM login WHERE id = '" & varID & "'")
                If database.dt.Rows.Count > 0 Then

                    'insert
                    updates("INSERT INTO `loginarchive`(`id`, `Fname`, `Sname`, `email`, `phonenum`, `bdate`, `password`, `verified`) 
                                 VALUES ('" & varID & "','" & varFname & "','" & varSname & "','" & varEmail & "','" & varPhone & "','" & varBdatequery & "','" & varPass & "','" & varVerified & "')")

                    'delete
                    updates("DELETE FROM `login` WHERE id = '" & varID & "'")
